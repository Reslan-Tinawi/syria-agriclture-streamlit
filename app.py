...
crop_items_to_ignore = crop_production_stats.loc[crop_production_stats["n_years"] != 60, "Item"].values

crop_production_df = crop_production_df.loc[~crop_production_df["Item"].isin(crop_items_to_ignore)]
...
fig = px.line(
    data_frame=crop_production_df.loc[crop_production_df["Item"].isin(total_production_by_item.iloc[:15, 0])],
    x="Year",
    y="Value",
    color="Item",
